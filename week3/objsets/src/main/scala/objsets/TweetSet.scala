...
class Tweet(val user: String, val text: String, val retweets: Int) {
  def eq(that: Tweet): Boolean = this.user == that.user && this.text == that.text && this.retweets == that.retweets
...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet {
  ...
  private def mostRetweeted(a: Tweet, b: Tweet) =
    if (a.retweets > b.retweets) a
    else b

  override def mostRetweeted: Tweet = {
    val m = mostRetweeted(left.mostRetweeted, right.mostRetweeted)
    if (m.retweets > this.elem.retweets) m
    else elem
  }

  override def descendingByRetweet: TweetList = {
    val m = this.mostRetweeted
    val l = this.remove(m).descendingByRetweet

    if (l.isEmpty) new Cons(m, Nil)
    else new Cons(m, l)
  }
