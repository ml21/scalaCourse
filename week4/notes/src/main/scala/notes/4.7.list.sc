...
- пустого листа Nil
- операции конструирования (Cons) x :: xs, которая создает новый лист из первого (head) элемента и остальных элементов (tail)
...
List() - тоже, что и Nil
List(2 :: xs) - этому паттерну будет соответсвовать лист, содержащий единственным элементом другой лист, который начинается с 2
*/
