...
object Anagrams {
  ...
  def wordOccurrences(w: Word): Occurrences =
    w.toLowerCase.groupBy(x => x).map(m => (m._1, m._2.length)).toList.sortBy(e => e._1)
  ...
  def wordAnagrams(word: Word): List[Word] =
    dictionaryByOccurrences getOrElse(wordOccurrences(word), "not found" :: Nil)
  ...
  def sentenceAnagrams(sentence: Sentence): List[Sentence] = {
    def internal(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(Nil)
      else for {
        combination <- combinations( occurrences )
        word <- dictionaryByOccurrences getOrElse (combination, Nil)
        sentence <- internal( subtract(occurrences,wordOccurrences(word)) )
        if !combination.isEmpty
      } yield word :: sentence
    }

    internal( sentenceOccurrences(sentence) )
  }
}
